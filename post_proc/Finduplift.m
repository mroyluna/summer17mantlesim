...
% Mousumi Roy, March 4, 2014; modified to include dynamic topography Feb
% 2015
...
Hcrust = 1000 - 30;  % crustal thickness in km; surface is at Yr = 1000 km
tcont  = [300:100:1600];
...
kovermu = 1e-15/1e-2;
startx = [1:20:990]'; %note here units must be in km as 
                       %displayed in box
starty = 500*ones(length(startx),1);
starty = 700*ones(length(startx),1);
...
%scale to real dimensional values
    ...
    if ii==1 
        ...
        Ymantle  = (Yr < Hcrust) & (Yr >= 800 );% & (Tr <= Tval);
    end
